...
import com.mksolution.depressionreducer.databinding.ActivityAdvanceBreathingBinding

class AdvanceBreathingActivity : AppCompatActivity() {
    private lateinit var binding: ActivityAdvanceBreathingBinding

    private val handler = Handler(Looper.getMainLooper())
    private val sectionColors = arrayOf(Color.GREEN, Color.YELLOW, Color.BLUE)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityAdvanceBreathingBinding.inflate(layoutInflater)
        ...
        binding.backAdvanceBreath.setOnClickListener {
            finish()
        }
    ...
    private fun startBreathingAnimation() {
        ...
        playSound(isHold = false) // Play inhale sound
        ...
        handler.postDelayed({
            ...
            when (currentSection) {
                1 -> {
                    // Hold for 7 seconds
                    changeProgressBarColor(sectionColors[currentSection])
                    playSound(isHold = true) // Play hold sound
                    animateProgressBar(7000)
                }
                2 -> {
                    // Exhale for 8 seconds
                    changeProgressBarColor(sectionColors[currentSection])
                    playSound(isHold = false) // Play exhale sound (same as inhale)
                    ...
                    currentSection = 0
                    changeProgressBarColor(sectionColors[currentSection])
                    playSound(isHold = false) // Restart inhale sound
    ...
    private fun playSound(isHold: Boolean) {
        if (mediaPlayer == null) {
            mediaPlayer = MediaPlayer.create(this, if (isHold) R.raw.servantbell else R.raw.copperbell)
        } else {
            mediaPlayer?.reset()
            mediaPlayer = MediaPlayer.create(this, if (isHold) R.raw.servantbell else R.raw.copperbell)
        }
